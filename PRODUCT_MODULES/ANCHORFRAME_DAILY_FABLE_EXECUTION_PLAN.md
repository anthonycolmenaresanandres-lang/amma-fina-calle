...
- **D3 References:** **Received + cropped** (2026-07-07) — Anthony provided **5 photos**. Cropped to head-and-shoulders (identifier-stripped: no name/worksheet, no jersey crest/"COLOMBIA" text, no TV, no home layout); each crop visually QA'd. Held in **session scratchpad only** (`scratchpad/anchor_one_refs/crops/`), **NOT committed to git**. Nothing uploaded to any AI generation tool.
  - **Durable storage:** an empty Drive folder "AnchorFrame Daily - Anchor One references (cropped, private)" was created in Anthony's Google Drive (id `1Gml5Ypv7yNXV7KycVv2Pjzzj3A1MtFTZ`). Direct MCP upload was **not possible** — the only upload path routes the image as inline base64 through the transcript, which the environment's privacy classifier correctly blocks for a minor's photo. So the **5 cropped files were delivered to Anthony directly** (SendUserFile) for him to drop into that Drive folder from his own device — which is also the highest-quality, safest source for the eventual ChatGPT upload.
- **D3b Second batch — REJECTED (2026-07-07), not stored, not used:** A follow-up set of 4 images was reviewed and discarded. Reasons: one was a **group party photo containing several other people's children** (third-party minors — not ours to process); one was a **full-body outdoor shot in front of the family home** (reveals home/address context, face too small); two were **party-invitation / phone-screenshot graphics** carrying heavy personal identifiers (child's first name + age, a venue street address, an RSVP phone number, a date/schedule, and a third party's name + private messages). None meet the reference spec and all conflict with the privacy guardrails. Working copies were deleted from scratchpad; nothing committed, nothing uploaded. Take-away for Anthony recorded below (need plain head-shots, no other children, no home/address, no invitations/screenshots).
- **D4 Reference count/type:** 5 casual home snapshots (batch 1). Framing audit: **1 strong frontal, eyes open** (usable); 1 frontal but eyes squeezed shut (laughing); 2 profile/¾ with eyes downcast; 1 back-of-head (no face). GAP: need cleaner neutral **straight-on + ¾-left + ¾-right, eyes open, even light, plain background**. All shots include home interior / national-team jersey / a wall worksheet with handwriting — **must crop to head-and-shoulders before any tool use** so no home/name/uniform identifiers travel with the reference.
