...
_Created: 2026-06-07 | Version: 1.1_
_v1.1: registered the **Campaign Pack** tier (fixed Stadium Shell + one behind-goal ad zone + player/keeper kit recolor)._
...
5b. **Logos are approved overlays only — never AI-generated.** A client/brand logo is
   always the client's real, approved logo file from intake; never AI-recreated,
   traced, or regenerated. AI may make surrounding promo art, never the logo mark.
...
> **Productized path:** the **Campaign Pack** (see the dedicated section below)
> packages the standardized Stadium Shell offer — **behind-goal ad zone + player
> kit + keeper kit** on a fixed shell. It is the repeatable, sell-once/run-many
> form of these tiers and is the default way new Penalty Shootout clients are run.

---

## Campaign Pack — the standardized Penalty Shootout product

> **Stadium Shell stays fixed. Only a Campaign Pack changes per client.**
> This is the productized model for Penalty Shootout: we are **not** building
> unlimited customization or a new game per client. We ship **one fixed game
> shell** and swap a small, typed **Campaign Pack** on top of it.
>
> Specs: `ASSET_SPECS/PENALTY_AD_ZONE_SPEC.md`, `ASSET_SPECS/PENALTY_KIT_SPEC.md`.

### Fixed Stadium Shell (frozen — never changes per client)

The shell is company-owned product baseline and is **not** edited for a campaign:

- Core **game engine**, scoring, keeper AI, match state (the frozen `engine/` layer).
- **Goal & net geometry**, the six aim zones, **field & perspective**, gameplay layout.
- Shooting / game logic and the overall **shell**.
- **Top scoreboard / menu strip** design and **score placement** — fixed shell chrome.
  (The strip may carry a fixed menu-button **label/link**, but it is **not** a
  per-campaign creative zone right now.)

A Campaign Pack may **never** change the engine, the zones, the geometry, the
gameplay, or the routes.

### Campaign Pack — the three (and only) customizable slots

| # | Slot | What changes | Spec |
|---|------|--------------|------|
| 1 | **Behind-goal ad zone** | The one large dynamic campaign panel behind the goal (promo / seasonal / hero art). Renders behind the goal frame; must preserve gameplay readability. | `ASSET_SPECS/PENALTY_AD_ZONE_SPEC.md` |
| 2 | **Player (kicker) shirt** | Recolor the mascot kicker's kit (layered tintable kit). | `ASSET_SPECS/PENALTY_KIT_SPEC.md` |
| 3 | **Keeper shirt** | Recolor the Sentinel Keeper's kit (layered tintable kit). | `ASSET_SPECS/PENALTY_KIT_SPEC.md` |

**Out of scope for the current Campaign Pack:** ❌ no keeper message box · ❌ no
bottom promo text strip · ❌ the top strip is **not** a per-campaign zone.

### Campaign Pack rules (absolute)

- **Non-human mascots only.** Kicker and keeper stay original Fina Calle mascots;
  shirt customization = **recoloring the mascot kit**, never a real player or club kit.
- **No real club / FIFA / World Cup / federation / real-face branding** — anywhere,
  including the ad zone. (Inherited from the Guardrails section.)
- **Logos are approved overlays only — never AI-generated.** Any brand/client logo
  used in a campaign (ad zone, watermark) must be the **client's real, approved logo
  file** supplied through intake — never recreated, traced, or generated by AI. AI
  may produce decorative/promo art around it, but **the logo mark itself is always
  the approved original.**
- **Client approval required in writing before publish.** A campaign is
  **PENDING CLIENT APPROVAL** until signed off; it does not deploy to production.
- **Primitive / default fallback always.** Missing ad image → default backdrop;
  missing kit color → default kit. A campaign is never a broken/404 look.
- **One shell, many campaigns.** Adding a client = a new Campaign Pack (config +
  approved ad image + kit colors), **not** new game code.

### Supersession note — LED/banner → behind-goal ad zone (read before merging Stadium ad PRs)

The Campaign Pack's **behind-goal ad zone** is now the **canonical ad placement**.
It **supersedes** the earlier exploratory ad/banner ideas in the Stadium work:

- The small **`ledBanner`** (black LED strip behind the keeper), the **`scoreBug`**
  promo box, and the lower **`adBanner`** strip were R&D toward the right model;
  the **single large behind-goal ad zone** replaces them as the product direction.
- **Do not merge any open Stadium ad/banner PR blindly into production as the final
  ad model.** (At time of writing, the LED/scoreBug + ball/kicker work was
  consolidated into a draft PR after the prior per-feature PRs were closed.) Any such
  PR must be re-evaluated against this Campaign Pack model — keep what fits the fixed
  shell (e.g. the photo backdrop, fixed scoreboard strip), and fold the ad concept
  into the **behind-goal ad zone** rather than shipping the small LED/banner as the
  customizable surface.
- This is a **docs/process decision only** — no app code changes here. The renderer
  changes happen later, in their own PRs, after this model is company law.
