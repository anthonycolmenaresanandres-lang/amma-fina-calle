# Operating Model — AMMA / Fina Calle (how we run)
...
| Fallback | Does more hands-on coding **only when Codex is out of tokens/down** | — | — |

## Handoff protocol
1. **Claude** diagnoses → decides → writes a precise, self-contained Codex prompt into `OPERATIONS/CODEX_QUEUE.md` (paths, IDs, guardrails, acceptance check).
...
4. **Anthony** does only the human-gated steps (secrets, approvals, access, publish) — see below.

## Source-of-truth map (where things live)
- **Code:** GitHub `anthonycolmenaresanandres-lang/amma-fina-calle`, app in `APP/web`. Canonical local clone = this folder. `main` = production.
- **Prod app:** Vercel project `amma-fina-calle` → `finacalleos.com`. Deploys on push to `main`.
...
## What ONLY Anthony can do (explicit — agents will never do these)
...
2. Separate: references · logos · menus · brand notes · QR assets · web copy · game/package ideas.
3. From minimal info, Claude drafts: client profile → menu structure → brand direction → QR menu/sign package → website starter → next-step checklist (Draft 1).
4. Anything that needs code → Codex prompt in `CODEX_QUEUE.md`. Anything repeatable → clone/bot.
...
- **One step from automatic:** request → **email notification** (needs `RESEND_API_KEY` in Vercel; 2 of 3 vars set — see Anthony action list).
