...
export class CalComConnector implements BookingConnector {
  ...
  private cfg: NonNullable<Tenant["calcom"]>;
  private business: Tenant["business"];

  constructor(tenant: Tenant) {
    this.cfg = tenant.calcom!; // factory only builds this when configured
  ...
  async checkAvailability({ date }: { date: string }): Promise<Slot[]> {
    ...
    const url = `${this.cfg.baseUrl}/slots?eventTypeId=${this.cfg.eventTypeId}` +
      `&start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}&timeZone=${encodeURIComponent(this.cfg.timeZone)}`;
    const res = await withTimeout(url, { headers: this.headers() });
    if (!res.ok) return [];
  ...
  async book({ slot, service, customer, idempotencyKey }: { slot: Slot; service: string; customer: Customer; idempotencyKey: string }): Promise<BookingResult> {
    ...
    const body = {
      start: slot.startIso,
      eventTypeId: this.cfg.eventTypeId,
      attendee: { name: customer.name ?? "Phone guest", email, timeZone: this.cfg.timeZone, phoneNumber: customer.phone },
      metadata: { service, source: "fina-calle-voice", idempotencyKey },
    };
    const res = await withTimeout(`${this.cfg.baseUrl}/bookings`, { method: "POST", headers: this.headers(), body: JSON.stringify(body) });
