 ...
 *
 * `demo` mode runs a deterministic client-side match against the passed items
 * so the experience is fully playable on the login-free preview. On the live
 * dashboard (demo=false) the same shell hands off to the server triage actions
 * (wired next); for now it shows the inviting bar.
 */
...
function parse(text: string, items: Item[]): Result {
  ...
  const found = items.find((it) => t.includes(` ${it.name.toLowerCase()} `) || t.includes(it.name.toLowerCase()));
  ...
  if (found && priceMatch && /(\$|\bto\b|\bprice\b|\bnow\b|\bmake\b|dollar)/.test(t)) {
    ...
    return {
      ...
      detail: `${money(found.price)} → $${next.toFixed(2)} — live on your menu.`,
    };
  }
  return {
    kind: "review",
    reason: "I’ll pass this to the Fina Calle team — they’ll handle it and follow up.",
  };
}

const CHIPS = ["86 the Flan Latte", "change Mocha to $8", "bring back Cortado"];

export default function AskBar({ items, demo = false }: { items: Item[]; demo?: boolean }) {
  ...
  function confirm() {
    if (result?.kind === "apply") setDone("✓ Done — it’s live on your menu. Customers see it now.");
    else if (result?.kind === "review") setDone("✓ Sent to the Fina Calle team. You’ll hear back.");
    setResult(null);
    setText("");
  }
          ...
          {CHIPS.map((c) =>
            demo ? (
              <button
                ...
                className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5 text-xs text-[#c8d0d4] transition hover:border-[#d8b36d]/50 hover:text-white"
              >
                {c}
              </button>
            ) : (
      ...
      {result?.kind === "apply" ? (
        <div className="mt-3 rounded-2xl border border-[#d8b36d]/30 bg-[#d8b36d]/8 px-4 py-3">
          ...
          <div className="mt-3 flex gap-2">
            <button type="button" onClick={confirm} className={buttonClass("primary")}>
              Confirm change
            </button>
